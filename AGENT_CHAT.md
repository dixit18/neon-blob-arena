...
**Riya (receipts):** build green (JS hash unchanged — pure CSS/HTML proof) + 44/44 web + BROWSER GREEN on the ART-1 build + eyes on 1280 (new face confirmed) and 360. HONEST LIMIT logged: headless enforces ~500px minimum viewport (evidence: identical clip pre/post across builds, media query math verified correct), so TRUE 360 device truth needs a real phone — user's hands. Base hero min lowered anyway as belt-and-suspenders (zero desktop impact).
**Rehan (counter, signed):** grain-over-everything + display font could still read "template with texture" — ANSWERED in part (type + sticker + grain + composition move together; no single trick carries it), and the final judge is the user's eyes, not mine.
**Aarav:** ART-1 DONE. Program resumes DV-3 unless the user's eyes redirect.
**Kai:** Committing + pushing.

## 2026-09-18 — Mira's de-slop direction (SG-5 research, cited)
**Mira (R&D harvest):** 1) TYPE AS HERO — neubrutalism.com + NN/g (Apr 2025): display typeface IS the design; bold type as focal point, asymmetry at macro level, alignment at micro. Pick: Unbounded (Google Fonts, OFL free commercial, variable 200–900) for display, system stack stays for body + Devanagari fallback. 2) STICKER GRAMMAR — our own GUMMY-GOTH heritage + sticker-pack portfolios: white/dark keylines, HARD offset shadows (4px solid, never blur glows), slight rotations, press physics (shadow collapses on tap). 3) GRAIN — the documented anti-slop signature (multiple 2026 production breakdowns): 35mm-style grain at low opacity over the finished frame kills the smooth-diffusion look; static (never animated — temporal stability rule from halftone-fx research). 4) HUD DISCIPLINE — WANDR (Jul 2026): only frequent+urgent earns pixels; our pills already pass, so landing keeps them, game HUDs untouched. 5) ARCADE MENU language — framedSelect/ring-selector/pixel-type lineage (Figma CC-BY arcade menu, Polycade referents): keep our portal rings as the selector metaphor, already ours.
**Kabir (counter, signed):** font CDN = render-block + FOUT + offline flop — ANSWERED: display=swap + preconnect + system fallback, headings only (body never waits). Rotations + hard shadows risk mess on 360px — ANSWERED: sub-degree on small screens, 360px paint-gated. Grain over UI text risks legibility + composite cost — ANSWERED: 6–7% static tile, pointer-events none, below dialogs. Emoji-as-icons stays for now (100+ call sites = ART-2, not this slice).
**Aarav:** ART-1 scoped: index.html CSS + font link + grain div ONLY. Zero JS changes, zero canvas changes, zero behavior change. Proof by eyes at 1280 + 360.
**Leo:** building.
