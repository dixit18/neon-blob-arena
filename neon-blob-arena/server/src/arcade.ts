// VARIANT ARENAS — batch A of the 10-game day. One parameterized sim, three games:
// SUGAR RUSH (blitz config), KING HILL (hold center to score), TAG FRENZY (pass IT).
// Same transport + rounds + streaks + bots as the flagships; per-game rules are
// flags, not forks. New verbs here must earn a dedicated sim (see polar/buffet).
...
export interface VariantDef {
  ...
  speedMul: number;
  eatRatio: number;
  chomp: boolean; // tag passes IT instead of eating
  crownByScore: boolean; // hill + tag crown score, rush crowns mass
}
export const VARIANTS: Record<string, VariantDef> = {
  rush: { id: 'rush', roundTicks: 90 * 20, pelletTarget: 520, speedMul: 1.15, eatRatio: 1.0, chomp: true, crownByScore: false },
  hill: { id: 'hill', roundTicks: 180 * 20, pelletTarget: 340, speedMul: 1.0, eatRatio: 1.12, chomp: true, crownByScore: true },
  tag: { id: 'tag', roundTicks: 180 * 20, pelletTarget: 300, speedMul: 1.05, eatRatio: 99, chomp: false, crownByScore: true },
};
export const HILL = { x: 2000, y: 2000, r: 260 }; // static center hill (client mirrors)
...
export class VariantRoom {
  id: string;
  readonly game: GameId;
  readonly def: VariantDef;
  tick = 0;
  nextNum = 1;
  ...
  itId: string | null = null; // tag: who is IT
  itGraceUntil = 0; // tag: 1s no-repass grace
  grid = new Map<number, number[]>();

  constructor(game: GameId, id: string) {
    const def = VARIANTS[game];
    if (!def) throw new Error('not a variant game: ' + game);
    this.game = game;
    this.def = def;
    this.id = id;
    console.log(`[variant ${game} ${id}] created`);
    for (let i = 0; i < def.pelletTarget; i++) this.addPellet();
  }
  ...
  addPlayer(id: string, name: string, isBot = false): PlayerState {
    ...
    if (this.def.id === 'tag' && !this.itId) { this.itId = id; this.pushFeed(`🏃 ${st.name} is IT!`); }
    return st;
  }
  ...
  removePlayer(id: string) {
    ...
    if (this.def.id === 'tag' && this.itId === id) { // IT left: crown a successor
      const next = [...this.players.values()].find(o => o.alive);
      this.itId = next ? next.id : null;
      this.itGraceUntil = this.tick + 40;
      if (next) this.pushFeed(`🏃 ${next.name} is IT!`);
    }
  }
  ...
  handleInput(id: string, dx: number, dy: number, dash = false) {
    ...
    const speed = speedForMass(p.mass) * this.def.speedMul;
  ...
  updateBots() {
    const it = this.itId ? this.players.get(this.itId) ?? null : null;
    for (const p of this.players.values()) {
      if (!p.isBot || !p.alive) continue;
      if ((this.tick + p.hue) % 2 !== 0) continue; // cheap AI @10Hz
      let dx = 0, dy = 0, dash = false;
      if (this.def.id === 'tag' && it && it.alive) {
        if (it.id === p.id) { // IT chases nearest
          ...
          if (best) { dx = best.x - p.x; dy = best.y - p.y; dash = Math.hypot(dx, dy) > 200 && this.tick >= p.dashCdUntil; }
        } else { // survivors flee IT inside 600px, else graze
          const d = Math.hypot(it.x - p.x, it.y - p.y);
          if (d < 600) { dx = p.x - it.x; dy = p.y - it.y; dash = d < 260 && this.tick >= p.dashCdUntil; }
        }
      }
      if (dx === 0 && dy === 0) {
        if (this.def.id === 'hill') {
          const hd = Math.hypot(HILL.x - p.x, HILL.y - p.y);
          if (hd > HILL.r * 0.7) { dx = HILL.x - p.x; dy = HILL.y - p.y; }
        }
        if (dx === 0 && dy === 0) {
          ...
          if (this.def.chomp) {
            for (const o of this.players.values()) {
              ...
              if (p.r > o.r * this.def.eatRatio && d < 550 && (!prey || d < Math.hypot(prey.x - p.x, prey.y - p.y))) prey = o;
            }
          }
  ...
  step() {
    ...
    if (!this.def.chomp) this.tagPass(); // BEFORE collide: separation parks pairs
    this.collide(); // out of tag reach — contact must pass first (feel + correctness)
    this.eatPellets();
    if (this.def.chomp) this.eatPlayers();
    // scoring ticks: hill holders +2/s, tag survivors +1/s (20 ticks = 1s)
    if (this.tick % 20 === 0) {
      if (this.def.id === 'hill') {
        for (const p of this.players.values()) {
          if (!p.alive) continue;
          if (Math.hypot(HILL.x - p.x, HILL.y - p.y) < HILL.r) p.score += 2;
        }
      } else if (this.def.id === 'tag') {
        for (const p of this.players.values()) {
          if (!p.alive || p.id === this.itId) continue;
          p.score += 1;
        }
      }
      let m = 0, n = 0;
    ...
    this.roundTick++;
    if (this.roundTick >= this.def.roundTicks) this.endRound();
    while (this.pellets.length < this.def.pelletTarget) this.addPellet();
    this.botTimer++;
  ...
  tagPass() {
    if (!this.itId || this.tick < this.itGraceUntil) return;
    const it = this.players.get(this.itId);
    if (!it || !it.alive) return;
    for (const o of this.players.values()) {
      if (o.id === it.id || !o.alive) continue;
      if (this.tick < o.shieldUntil) continue; // fresh spawns get a breather
      if (Math.hypot(o.x - it.x, o.y - it.y) < (it.r + o.r) * 0.7) {
        this.itId = o.id;
        this.itGraceUntil = this.tick + 20;
        this.pushFeed(`🏃 ${o.name} is IT!`);
        break;
      }
    }
  }
  ...
  eatPlayers() {
    ...
    for (const eater of list) {
      ...
      for (const victim of list) {
        ...
        if (eater.r < victim.r * this.def.eatRatio) continue;
  ...
  endRound() {
    ...
    const scoreOf = (p: PlayerState) => this.def.crownByScore ? p.score : p.mass;
    const champ = alive.filter(p => !p.isBot).sort((a, b) => scoreOf(b) - scoreOf(a))[0]
      ?? alive.sort((a, b) => scoreOf(b) - scoreOf(a))[0];
    if (champ) {
  ...
  snapshot(forId: string): ServerSnapshot {
    ...
    for (const p of this.players.values()) {
      ...
      players.push({ id: p.id, n: p.name, x: Math.round(p.x * 2) / 2, y: Math.round(p.y * 2) / 2, r: Math.round(p.r * 10) / 10, h: p.hue, k: p.kills, s: Math.floor(p.mass), b: p.isBot ? 1 : 0, ht: this.def.id === 'tag' && p.id === this.itId ? 1 : 0, c: 0 });
    }
    ...
    return {
      ...
      me: me ? {
        ...
        it: this.def.id === 'tag' && forId === this.itId ? 1 : 0,
      ...
      round: Math.max(0, Math.ceil((this.def.roundTicks - this.roundTick) / TUNE.TICK_HZ)),
      orbs: [],
      v: this.def.id === 'hill' ? { zone: { ...HILL } } : this.def.id === 'tag' ? { it: this.itId ?? undefined } : undefined,
    };
  }
}
